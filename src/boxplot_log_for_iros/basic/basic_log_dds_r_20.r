...
ggp = all_data %>%
    ...
    geom_boxplot(outlier.shape=NA, na.rm=TRUE,coef=1)+ # Display boxplot
    ...
    #y-axis
    scale_y_continuous(
        breaks=c(-40,-20,-5,-0.1,0,0.1,5,20,40), #Specify the scale to be displayed
        trans=pseudo_log_trans(sigma = 0.02, base = exp(1)), #Decrease sigma and increase log slope.
        name="Jitter [ms]",
        # limits=c(-13.13)
    )+
    #x-axis
...
plot(ggp)
ggsave("/home/ryuko/perf_test_ws/src/performance_test/performance_test/helper_scripts/boxplot_log_for_iros/basic/basic_dds_r_20.eps",width=25,height=9)
